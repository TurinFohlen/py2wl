...
class WolframKernel:
    ...
    def _launch_parallel_kernels(self):
        ...
        try:
            ...
            # 子内核就绪后设置编译优化，主内核 + 所有子内核同步生效
            self._apply_compile_options()
        except Exception as e:
            self._parallel_launched = False   # 失败时复位，允许下次重试
            log.warning(f"启动并行内核失败（不影响主内核）: {e}")

    def _apply_compile_options(self):
        """
        设置全局 Compile 优化选项，对所有后续 Compile[] 调用生效。

        CompileTarget -> "C"：
            将 Compile[] 生成的代码编译为原生 C，再由系统 C 编译器（gcc/clang）
            编译为共享库。在 ARM Android 上实测比默认 WVM（Wolfram 虚拟机）
            快 2-4×。

        CompileFallback -> True：
            C 编译失败（缺少编译器、不支持的语法等）时自动退回 WVM，
            保证不会因为编译失败而报错崩溃。

        OptimizationLevel -> 3：
            等价于 gcc -O3，开启循环展开、向量化、内联等全部优化。
            在"Speed"优先场景下叠加 CompileTarget C 可再提速约 2-3×。

        两者相乘：C 编译 × O3 ≈ 4-9× vs 默认 WVM，在 ARM 上尤其显著。

        注意：此选项只对经过 Compile[] 的自定义函数及内部使用 Compile 的
        WL 函数有效。Dot/Eigensystem 等内置函数已经是预编译 BLAS/LAPACK，
        不受此选项影响。
        """
        try:
    ...
    def evaluate_to_file(self, expr: str, fmt: str = "wxf",
        ...
        # ── 无头模式光栅化 ────────────────────────────────────────
        # Wolfram Engine 无图形前端时，Plot/Plot3D 等返回符号 Graphics 对象。
        # Export 遇到符号对象只会把文字表示写入文件，得到的不是真正的图片。
        # 对图像格式自动在内核侧包一层 Rasterize[]，强制光栅化为像素数据。
        # 用户可通过 rasterize=False 参数跳过（处理已是 Image 对象的表达式时）。
        _IMAGE_FMTS = {"PNG", "JPEG", "GIF", "BMP", "TIFF"}
        if wl_fmt in _IMAGE_FMTS:
            _raster_size = os.environ.get("PY2WL_RASTER_SIZE", "1920x1080")
            _w, _h = _raster_size.lower().split("x")
            expr = f"Rasterize[{expr}, RasterSize->{{{_w}, {_h}}}]"
