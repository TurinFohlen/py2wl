    ...
    def _call_with_fault(self, path, args, kwargs, rule=None):
        ...
        try:
            if oc == "from_wl_image":
                ...
                #
                # 无头模式（headless）下 Wolfram Engine 没有图形前端，
                # Plot/ListPlot/Plot3D 等函数返回符号 Graphics 对象而非像素，
                # Export 会把对象的文字表示写入文件而不是真正的图片。
                #
                # 解决：内核侧用 Rasterize[] 强制光栅化，把符号图形转成像素矩阵，
                # Export 拿到的才是真正的图像数据。
                #
                # 分辨率优先级：
                #   1. 规则字段 raster_size（如 [1920, 1080]）
                #   2. 环境变量 PY2WL_RASTER_SIZE（如 "1920x1080"）
                #   3. 默认 1920x1080
                fmt = rule.get("image_format", "png")
                _default_raster = os.environ.get("PY2WL_RASTER_SIZE", "1920x1080")
                _w, _h = (int(x) for x in _default_raster.lower().split("x"))
                _raster = rule.get("raster_size", [_w, _h])
                _raster_wl = f"{{{_raster[0]}, {_raster[1]}}}"
                core_expr = f"Rasterize[{core_expr}, RasterSize->{_raster_wl}]"
